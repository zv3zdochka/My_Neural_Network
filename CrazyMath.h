...
// helpers
    ...
    template<class F>
    typename std::enable_if<!std::is_arithmetic<F>::value, Subtract<F, Const>>::type
    operator-(const F &f, double value) {
        return Subtract<F, Const>(f, Const(value));
    }

    template<class F>
    typename std::enable_if<!std::is_arithmetic<F>::value, Subtract<Const, F>>::type
    ...
    template<class F>
    typename std::enable_if<!std::is_arithmetic<F>::value, Multiply<F, Const>>::type
    operator*(const F &f, double value) {
    ...
    template<class F>
    typename std::enable_if<!std::is_arithmetic<F>::value, Divide<F, Const>>::type
    operator/(const F &f1, double value) {
...
// derivatives
    ...
    public:
        Derivative<Exponent<F> >(const Exponent<F> &f)
                : m_base(f.m_base), m_factor(log(f.m_base)), m_f(f.m_f), m_df(f.m_f) {
        }
