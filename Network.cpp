...
void Network::train(std::vector<std::vector<std::vector<float>>> data, const std::vector<std::vector<float>> &answer,
    ...
    for (int epoch = 0; epoch < epochs; ++epoch) {
        ...
        for (auto &j: data) {
            ...
            std::vector<std::vector<float>> demi_er;
            for (int lay = network.size() - 1; lay > 0; lay++) {
                ...
                if (k == 1) {
                    ...
                    demi_er = divide(Matrix(semi).transpose(), errors).getData();
                } else {
                    Matrix(synapse[lay-1]).transpose().showMatrix("fur");
                    Matrix(errors).showMatrix("huh");
                    demi_er = divide(Matrix(synapse[lay-1]).transpose(), errors).getData();
                }

                Matrix(demi_er).showMatrix("gggggg");
            }
...
void Network::save(const char *filename) const {
    ...
    if (layers > 0) {
        nlohmann::json &last_layer_data = output.emplace_back();

        last_layer_data[NETWORK_NUM_NEURONS_NAME] = network[0].size();
        last_layer_data[NETWORK_ACTIVATION_FN_NAME] = function_type_to_string(network.back()[0].fn_type);
    }
